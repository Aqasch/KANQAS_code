...
class MovingThreshold:
    def __init__(self, config, **kw):
        ...
        self.lowest_cost = self.min_en + self.current_threshold
    ...
    def reduce_amortisation(self):
        if self.success_thresh:
            ...
            if self.success_counter >= self.success_thresh and self.radius_shift_counter < self.succ_radius_shift and self.succes_switch > abs(self.min_en - self.lowest_cost):
    ...
    def greedy_shift(self):
        ...
        if self.call_counter > 10 and (self.call_counter%(self.greedy_shift_time)) == 0:
            if self.amortisation:
                self.current_threshold = abs(self.min_en - self.lowest_cost) + self.amortisation

                if self.success_thresh:
                    self.radius_shift_counter = 0
                    self.success_counter = 0
            else:
                self.current_threshold = abs(self.min_en - self.lowest_cost)
